...
var PriceQueue = {
                        ...
                        if (response && response.success && response.listinginfo) {
                            ...
                            if (v_currencyformat && GetCurrencyCode) {
                                ...
                                for (var i in response.listinginfo) {
                                    if (response.listinginfo[i].converted_price) {
    ...
    StartQueue: function () {
        ...
        if (PriceQueue._urls.length > 0) {
            ...
            //    url: url,
            //    success: function (response, textStatus, jqXHR) {
            //        PriceQueue._cache[this.url] = { response: response };
            //        if (this.url == PriceQueue._urls[0]) {
            //            PriceQueue._urls.shift();
            //        }
            //        if (PriceQueue._queue[this.url]) {
            //            var handlers = PriceQueue._queue[this.url].handlers;
            ...
            //                }
            //            }
            //            delete PriceQueue._queue[this.url];
            //        }
            //        $J('#_priceQueueCont .pq-progress').css({ width: '1%' }).animate({ width: '100%' }, PriceQueue._successDelay);
            //        window.setTimeout(function () { PriceQueue._isRunning = false; PriceQueue.StartQueue(); }, PriceQueue._successDelay);
            //    },
            //    error: function () {
            //        $J('#_priceQueueCont .pq-progress').css({ width: '1%' }).animate({ width: '100%' }, PriceQueue._failureDelay);
